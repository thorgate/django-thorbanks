...
def request_digest(request, bank_name):
    """
    ...
    for value in request:
        ...
        digest += unicode(value_len).rjust(3, u'0')
        digest += unicode(value)
    return digest.encode(settings.get_encoding(bank_name))


def create_signature(request, bank_name):
    """
    ...
    digest = request_digest(request, bank_name)
    key_binary = the_key.sign(SHA.new(digest))

    return b64encode(key_binary)


def verify_signature(request, bank_name, signature):
    """
